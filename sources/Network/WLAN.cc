...
void WLAN::WLAN_header::cp2buff(char *buff) const noexcept {
  ...
  std::memmove(buff + dest.size(), src.addr.data(),
               dest.size());
...
void WLAN::send_ARP_request(const char *iface, const char *targetIP) {
  ...
  memcpy(ethHeader.src.addr.data(), "\xff\xff\xff\xff\xff\xff", MACAddress::size());
  memcpy(ethHeader.dest.addr.data(), ifr.ifr_hwaddr.sa_data, MACAddress::size());
...
bool WLAN::parse_ARP_response(const char *buffer) {
  ...
  for(std::size_t i = 0, end = MACAddress::size(); i < end; ++i) {
    t_ssf << std::setfill('0') << std::setw(2) << std::right << std::hex << (int)arpHeader->senderMac[i] ;
    t_ssf << ((i + 1 != end) ? ":" : "");
  }
...
void WLAN::scan_subnet(void) {
  ...
  for (std::size_t i = 0, end = sizeof(sourceIP); i < end; ++i) sourceIP[i] = 0;
  strcpy(sourceIP, inet_ntoa(((sockaddr_in *) &ifr.ifr_addr)->sin_addr));  
